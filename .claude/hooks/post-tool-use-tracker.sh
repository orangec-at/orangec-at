...
# This runs after Edit, MultiEdit, Write, or TodoWrite tools complete successfully
...
# ============================================
# TODO Auto-Sync: todowrite → dev/active/TODO.md
# ============================================
if [[ "$tool_name" == "TodoWrite" || "$tool_name" == "todowrite" ]]; then
    todo_file="$CLAUDE_PROJECT_DIR/dev/active/TODO.md"
    
    # Ensure directory exists
    mkdir -p "$(dirname "$todo_file")"
    
    # Generate TODO.md from todowrite input
    {
        echo "# Current TODOs"
        echo ""
        echo "> Auto-generated by post-tool-use hook. Do not edit manually."
        echo "> Last updated: $(date '+%Y-%m-%d %H:%M:%S %Z')"
        echo ""
        
        # Parse todos from tool_input
        echo "$tool_info" | jq -r '.tool_input.todos[]? | 
            if .status == "completed" then "- [x] \(.content)"
            elif .status == "in_progress" then "- [→] \(.content) *(in progress)*"
            elif .status == "cancelled" then "- [~] ~~\(.content)~~ *(cancelled)*"
            else "- [ ] \(.content)"
            end'
    } > "$todo_file"
