...
function Tabs({value, onValueChange, className, children, activationMode = 'auto'}: TabsProps) {
  return (
...
function TabsTrigger({value, className, children, disabled}: TabsTriggerProps) {
  const {value: activeValue, onValueChange, activationMode} = useTabsContext();
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    ...
    if (!disabled && activationMode === 'manual' && (e.key === 'Enter' || e.key === ' ')) {
      e.preventDefault();
      onValueChange(value);
    }
  };
