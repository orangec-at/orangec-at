...
const selectTriggerVariants = cva(
  `
  flex w-full items-center justify-between gap-(--gap-3) rounded-md border bg-background transition-all
  placeholder:text-(--krds-gray-40)
  disabled:cursor-not-allowed disabled:opacity-50
  data-[placeholder]:text-(--gray-40) data-[placeholder]:font-normal
  ...
  *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2
  ...
  `,
  {
    variants: {
      size: {
        small: "h-(--size-height-6) px-(--padding-6) text-sm",
        medium: "h-(--size-height-8) px-4 py-2.5 text-base",
        large: "h-(--size-height-10) px-6 py-3 text-lg",
      },
      state: {
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        selectTriggerVariants({ size, state: internalState, className })
      )}
      disabled={disabled}
      onFocus={handleFocus}
      onBlur={handleBlur}
