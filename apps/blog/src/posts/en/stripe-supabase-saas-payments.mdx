...
description: "Checkout sessions, webhooks, subscription status sync — the complete payment integration pattern for Supabase-backed SaaS."
date: "2026-02-20"
...
7. **Plan migration tested**
   - Upgrades and downgrades should preserve entitlements correctly.
   - Verify plan metadata in Stripe and your `stripe_price_id` sync logic stay aligned.

8. **Webhook retry behavior observed**
   - Trigger manual replay in Stripe dashboard.
   - Confirm your handler remains idempotent and the resulting row is stable.
