...
export const GadgetToolbar: React.FC<GadgetToolbarProps> = ({
  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    {
      role: "model",
      text: "안녕하십니까, 심야 서고의 관리자입니다. 아카이브의 기록물에 대해 궁금한 점이 있으신가요?",
    },
  ]);
  ...
  const tabs = [
    {
      ...
      icon: Footprints,
      label: "Log",
      color: "bg-[#e8e4d9] dark:bg-stone-800",
    ...
    },
    {
      ...
      icon: Highlighter,
      label: "Ink",
      color: "bg-[#e2ddd1] dark:bg-stone-700",
    ...
    },
    {
      ...
      icon: Music,
      label: "Focus",
      color: "bg-[#d6d3c9] dark:bg-stone-800",
    ...
    },
    {
      ...
      icon: Bot,
      label: "Librarian",
      color: "bg-[#ccc8bc] dark:bg-stone-900",
    ...
    },
    {
      ...
      icon: Mail,
      label: "Master",
      color: "bg-[#b5b1a2] dark:bg-red-950",
      border: "border-stone-500 dark:border-red-900",
      textColor: "text-stone-900 dark:text-red-100",
    },
    {
      id: "settings",
      icon: Settings,
      label: "User",
      color: "bg-[#fdfcf5] dark:bg-stone-900",
  ...
  const sendMessage = async (userMsg: string, isRetry = false) => {
    ...
    try {
      ...
      setChatHistory((prev) => [...prev, { role: "model", text: errorMsg, isError: true }]);
    } finally {
      clearTimeout(timeoutId);
  ...
  const handleSendDm = () => {
    ...
    setTimeout(() => {
      ...
      setDmContent("");
      alert("마스터의 서랍에 서신이 보관되었습니다. 확인 시 연락드리겠습니다.");
    }, 1500);
  };
  ...
  return (
    <div
      data-theme={theme}
      className="fixed bottom-0 left-0 right-0 z-[60] flex flex-col items-center pointer-events-none"
    >
            ...
            >
              <div className="p-6 border-b flex justify-between items-center bg-transparent backdrop-blur-sm sticky top-0 z-10 border-stone-200 dark:border-stone-800">
                <div className="flex items-baseline gap-3">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-stone-400 dark:text-stone-600">
                    Archives Index / v.1.2
                  </span>
                </div>
                <button
                  onClick={() => setIsExpanded(false)}
                  className="p-2 hover:bg-stone-800/10 rounded-full transition-colors"
                >
                          ...
                          <div className="p-4 rounded-lg bg-stone-100/50 dark:bg-stone-900/50 border border-stone-200 dark:border-stone-800">
                            <div className="flex items-center gap-2">
                              <div className="flex gap-1">
                                <div
                                  className="w-1.5 h-1.5 bg-stone-400 dark:bg-red-500 rounded-full animate-bounce"
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        >
                          {isMusicPlaying ? (
                            ...
                            <Play className="w-6 h-6 text-stone-500 ml-1" />
                          )}
                        </button>
                        <div>
                          ...
                          <p className="text-[10px] uppercase tracking-widest text-stone-500 font-bold">
                            {isMusicPlaying ? "Now Resonating" : "Mix Stagnant"}
                          </p>
                        </div>
                      </div>
                      {isMusicPlaying && (
                        ...
                        <div key={layer.id} className="space-y-3">
                          ...
                          <div className="h-1 rounded-full relative bg-stone-100 dark:bg-stone-900">
                            <input
                              ...
                              onChange={(e) =>
                                setVolumes({ ...volumes, [layer.id]: parseInt(e.target.value, 10) })
                              }
                ...
                {activeTab === "settings" && (
                  <div className="space-y-8">
                    <div className="p-10 border flex flex-col items-center gap-8 text-center shadow-sm relative overflow-hidden bg-white dark:bg-stone-900/50 border-stone-200 dark:border-stone-800">
                      ...
                      <div>
                        <p className="font-handwriting text-2xl mb-2">
                          {isLoggedIn ? "John Archivist" : "방문객 (Guest)"}
                        </p>
                        <p className="text-[10px] uppercase tracking-[0.2em] font-bold opacity-50">
                          인증 상태: {isLoggedIn ? "Academic Researcher" : "Guest Session"}
                        </p>
                      </div>
                    </div>

                    <div className="space-y-4">
                      <label className="text-[10px] uppercase tracking-[0.3em] font-bold text-stone-500 mb-2 block">
                        Display Mode
                      </label>
                    ...
                    <button
                      ...
                      className="w-full py-5 border-2 text-[11px] font-bold uppercase tracking-[0.3em] transition-all shadow-xl bg-stone-900 dark:bg-red-950 border-stone-900 dark:border-red-900 text-white dark:text-red-100 hover:bg-stone-700 dark:hover:bg-red-900"
                    >
                ...
                {activeTab === "steps" && (
                  <div className="space-y-12">
                    <div className="p-10 border-l-[12px] shadow-inner bg-[#f0f0f0] dark:bg-stone-900/50 border-stone-800 dark:border-red-900">
                      <span className="text-[10px] font-bold opacity-40 uppercase tracking-[0.4em] block mb-6">
                        Archive Reading Index
                      </span>
                      <div className="flex items-end gap-6">
                        <span className="text-7xl font-serif font-bold">
                          72<span className="text-2xl ml-1">%</span>
                        </span>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="p-6 border shadow-sm flex flex-col items-center bg-white dark:bg-stone-900 border-stone-200 dark:border-stone-800">
                        <span className="text-[9px] opacity-40 font-bold uppercase tracking-widest mb-3 block">
                          잉크 수집량
                        </span>
              ...
              <div className="h-6 border-t flex items-center justify-center bg-stone-100/50 dark:bg-stone-950 border-stone-200 dark:border-stone-900">
                <div
                  onClick={activeTab === "settings" ? onAdminSecret : undefined}
                  className="w-12 h-1 rounded-full cursor-pointer hover:bg-red-500 transition-colors bg-stone-300 dark:bg-stone-800"
                />
              </div>
            </motion.div>
          )}
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex-1 flex flex-col items-center justify-center transition-all duration-300 rounded-t-lg border-t border-x ${
                    tab.border
                  } shadow-sm ${tab.color} ${
                    isActive && isExpanded
                      ? "h-14 -translate-y-1 z-30 shadow-md"
                      : "h-11 hover:h-13 z-20 opacity-70"
                  }`}
                >
                  <tab.icon
                    className={`w-4 h-4 ${tab.textColor} ${
                      isActive && isExpanded ? "scale-110" : "scale-100"
