...
export default function ResponsiveHeader({
  ...
  const getHeaderClasses = () => {
    const baseClasses =
      "flex items-center mx-auto fixed left-0 right-0 z-50 transition-all duration-500 ease-out";
    const positionClasses = position === "top" ? "top-0" : "bottom-0";
    const visibilityClasses = isMobile ? "md:hidden" : "hidden md:flex";

    if (isMobile) {
      // Mobile: bottom fixed, no animation
      return `${baseClasses} ${positionClasses} ${visibilityClasses} justify-between mx-4 mb-4 px-6 py-4 bg-white/90 backdrop-blur-md shadow-lg border-t border-gray-100 rounded-full`;
    }

    // Desktop: top with scroll animation
    if (isScrolled) {
      return `${baseClasses} ${positionClasses} ${visibilityClasses} max-w-fit rounded-full mt-4 py-3 px-4 md:px-6 bg-white/80 backdrop-blur-md shadow-lg justify-center gap-2`;
    } else {
      return `${baseClasses} ${positionClasses} ${visibilityClasses} max-w-2xl lg:max-w-4xl mt-4 py-3 rounded-full px-4 md:px-6 bg-white/60 backdrop-blur-md shadow-lg border-t border-gray-100 justify-between`;
    }
  };
  ...
  return (
    <header className={getHeaderClasses()}>
      ...
      <nav className={`flex items-center ${isMobile ? "gap-2" : "gap-2"}`}>
        <div
